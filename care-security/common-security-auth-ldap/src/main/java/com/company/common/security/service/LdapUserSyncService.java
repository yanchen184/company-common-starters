 ...
 */
public class LdapUserSyncService implements LdapUserSyncer {
    ...
    /**
     * Sync LDAP user to local DB via SPI interface.
     */
    @Override
    @Transactional
    public SaUser syncUser(LdapUserResult ldapUser) {
        return syncUser(new LdapUserInfo(ldapUser.username(), ldapUser.displayName(), ldapUser.email()));
    }
