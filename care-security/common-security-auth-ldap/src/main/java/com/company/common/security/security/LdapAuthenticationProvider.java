 ...
 */
public class LdapAuthenticationProvider implements LdapAuthenticator {
    ...
    /**
     * Authenticate user against LDAP (SPI interface method).
     */
    @Override
    public Optional<LdapUserResult> authenticate(String username, String password) {
        return authenticateInternal(username, password)
                .map(info -> new LdapUserResult(info.username(), info.displayName(), info.email()));
    }
     ...
     */
    public Optional<LdapUserInfo> authenticateInternal(String username, String password) {
